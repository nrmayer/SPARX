import time
from automation import Automation2040W, SWITCH_A, INPUT_1
...
TIME_PER_TOGGLE = 1               # How much time to wait between each toggle (in seconds)
OUTPUT_NAMES = ("O1")   # The friendly names to give each digital output
...
toggle = True
index = 0
while not board.read_input(INPUT_1):
    time.sleep(0.1)
...
while not board.switch_pressed(SWITCH_A):
    ...
    for output_percentage in range(101):
        ...
        for i in range(1+0*board.NUM_OUTPUTS):
